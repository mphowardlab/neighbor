...
void lbvh_gen_codes(unsigned int *d_codes,
    ...
    if (max_block_size == UINT_MAX)
        ...
        hipFuncGetAttributes(&attr, reinterpret_cast<const void*>(kernel::lbvh_gen_codes<InsertOpT>));
...
void lbvh_bubble_aabbs(const LBVHData tree,
    ...
    if (max_block_size == UINT_MAX)
        {
        hipFuncAttributes attr;
        hipFuncGetAttributes(&attr, reinterpret_cast<const void*>(kernel::lbvh_bubble_aabbs<InsertOpT>));
        max_block_size = attr.maxThreadsPerBlock;
        }
    const unsigned int run_block_size = (block_size < max_block_size) ? block_size : max_block_size;
